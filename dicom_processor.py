...
def transform_to_hu(original, pixels):
    return original.RescaleIntercept + pixels * original.RescaleSlope
...
def generate(image_index, image_full_path, lesion_index, is_noise=False):
    ...
    scaled = noise = transform_to_hu(img, pixels)
    # 4x4 grid of noise
    ...
    image_folder = '../TFImageSet/images_{}x{}'.format(grid_dim, grid_dim)
    noise_folder = '../TFImageSet/noise_{}x{}'.format(grid_dim, grid_dim)
    image_number = 0
    save_file = '{}/{};{};{};{};{}.pickle'.format(image_folder, directories[image_index][8:], hu, 'lesion', size, image_number) if not is_noise else '{}/{};{}.pickle'.format(noise_folder, directories[image_index][8:], 'noise')
    save_path = image_folder if not is_noise else noise_folder
